...
interface PrivacyContentSectionProps {
  lang: string;
  isAr: boolean;
  dict: any;
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      for (const subsection of subsections) {
        const element = document.getElementById(`privacy-${subsection.id}`);
        if (element) {
            ...
            <div className="bg-[#FFF3ED] rounded-[2rem] p-6 sm:p-8 flex flex-col gap-6 shadow-sm border border-orange-100/50 text-left rtl:text-right">
              ...
              <div className="flex items-center gap-3 border-b border-orange-200/50 pb-4">
                ...
                <div>
                  ...
                  <span className="text-xs text-[#8C8C8C]">
                    {subsections.length} {isAr ? "أقسام الخصوصية" : "sections"}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-2 bg-white rounded-2xl p-5 border border-orange-100 shadow-sm space-y-3">
                ...
                <p className="text-xs text-[#8C8C8C] leading-relaxed">
                  {isAr
                    ? "فريقنا متواجد دائماً لمساعدتك والإجابة على أي أسئلة تخص سياسة الخصوصية وحماية بياناتك."
                    : "Our team is here to help you understand how we protect and manage your personal data."}
                </p>
                <Link
                  href={`/${lang}/help`}
            ...
            <div className="bg-[#FFF3ED] rounded-[2rem] p-8 sm:p-10 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm border border-orange-100 text-left rtl:text-right mt-10">
              <div className="space-y-2">
                ...
                <p className="text-xs sm:text-sm text-[#8C8C8C] max-w-xl">
                  {isAr
                    ? "فريق الدعم وحماية البيانات لدينا جاهز للإجابة على جميع استفساراتك وطلبات الخصوصية."
                    : "Our privacy and compliance team is available to assist you with data requests and questions."}
                </p>
              </div>
              <Link
                href={`/${lang}/help`}
